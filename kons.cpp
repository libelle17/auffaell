...
string umlweg(const string& u)
{
	string erg;
	for(auto it=u.begin();it<u.end();it++) {
		if ((unsigned char)*it==195) {
			it++;
			if ((unsigned char)*it==188) erg+="ue";
			else if ((unsigned char)*it==164) erg+="ae";
			else if ((unsigned char)*it==182) erg+="oe";
			else if ((unsigned char)*it==159) erg+="ss";
			else if ((unsigned char)*it==132) erg+="Ae";
			else if ((unsigned char)*it==150) erg+="Oe";
			else if ((unsigned char)*it==156) erg+="Ue";
			else erg+=*it;
		}
		else  erg+=*it;
	}
