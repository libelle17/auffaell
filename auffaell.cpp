...
{ 
  ...
  mdatei agd(dname,ios::out,0);
	if (agd.is_open()) {
		...
		RS ma(My,"SELECT persnr,kuerzel,nachname,vorname,if(aus=0,'',aus) FROM mitarbeiter WHERE kuerzel NOT IN ('gs','tk','') AND (ISNULL(aus) OR aus=0 OR aus>SUBDATE(NOW(),21))",0,ZDB);
		if (ma.obqueryfehler) {
